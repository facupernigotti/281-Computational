function [Va_Upwind,sb,sf] = vp_upwind(v0,par,num,grid,w,r)
...
Vab(1,:) = (r*grid.a(1,:) + par.e*w).^(-1); %state constraint boundary condition    


%consumption and savings with forward difference
cf = max(Vaf,1e-08).^(-1);
sf = par.e*w + r.*grid.a - cf ;
%consumption and savings with backward difference
cb = max(Vab,1e-08).^(-1);
sb = par.e*w + r.*grid.a - cb ;
